...
export interface StrategyResultV9 {
  signal: SignalV9;
  score: number;
  bullishScore: number;
  bearishScore: number;
...
/*
 * ==========================================
 * V9 — RESULT OF SYSTEMATIC BACKTEST TUNING
 * ==========================================
 *
 ...
 *   (see backtestAllStocksV9.ts / liveSignals),
...
/*
 * ==========================================
 * V9 — ADDED LIQUIDITY FILTER (new in this version)
 * ==========================================
 *
...
export function generateSignalV9(
  ...
  // ---- Pullback-to-EMA20 setup (research-backed, fires more often) ----
  // Instead of requiring peak momentum confirmation all at once, this
  // checks for: confirmed uptrend + price near (not far above) EMA20,
  // i.e. a shallow pullback within a trend, rather than a breakout.
  const pullbackDistance = Math.abs(candle.close - candle.ema20) / candle.ema20;
  const isNearEma20 = pullbackDistance <= 0.015; // within 1.5% of EMA20
  const rsiRecovering = candle.rsi >= 40 && candle.rsi <= 60; // recovering from a dip, not yet overbought

  metrics.push({
    label: "Pullback to EMA20",
    value: `${(pullbackDistance * 100).toFixed(2)}% from EMA20`,
    status: isNearEma20 ? "pass" : "neutral",
    threshold: "within 1.5% of EMA20",
  });

  const pullbackBuySetup =
    trendIsStrong &&
    bullishTrend &&
    isNearEma20 &&
    rsiRecovering &&
    candle.macdHistogram > (previousCandle?.macdHistogram ?? candle.macdHistogram) &&
    liquidityOk;

  if (pullbackBuySetup) {
    reasons.push("Pullback to EMA20 in confirmed uptrend");
  }

  let signal: SignalV9 = "HOLD";

  // Path A: original momentum-confirmation entry (strict, high-conviction)
  const momentumBuySetup =
    trendIsStrong &&
    bullishTrend &&
    candle.rsi >= 50 &&
    candle.rsi <= 70 &&
  ...
  if (momentumBuySetup || pullbackBuySetup) {
    signal = "BUY";
  } else if (
    // SELL now held to the SAME rigor as BUY — momentum and liquidity
    // are both required, matching what BUY has always required. This
    // was NOT the case before, which made SELL fire far more easily.
    trendIsStrong &&
    bearishTrend &&
    bearishMacd &&
    candle.rsi < 45 &&
    ...
    candle.macdHistogram < previousCandle.macdHistogram &&
    bearishScore >= 7
  ) {
    signal = "SELL";
  }
 ...
 * (V5, V6, and every V9 tuning pass). Kept as
 ...
 */
export const V9_EXCLUDED_STOCKS = [
  "YESBANK.csv",
  "NHPC.csv",
  "PNB.csv",
  "IDEA.csv",
