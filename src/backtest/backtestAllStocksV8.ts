...
// Live/production universe — single source of truth
// in src/config/stocks.ts, so adding a stock there
// automatically includes it here too.
const STOCKS = LIVE_STOCKS;
...
function backtest(
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    if (!inTrade) {
      ...
      const signal = generateSignalV8(candle, previousCandle);
      ...
      stopLoss = entryPrice - candle.atr * V8_RISK_PARAMS.atrStopMultiplier;
      target = entryPrice + candle.atr * V8_RISK_PARAMS.atrTargetMultiplier;
...
async function main() {
  console.log("");
  console.log("======================================");
  console.log("V8 BACKTEST — TUNED STRATEGY");
  console.log(`(ADX20 + 1-candle MACD + SL${V8_RISK_PARAMS.atrStopMultiplier}xATR/TG${V8_RISK_PARAMS.atrTargetMultiplier}xATR + next-open entry)`);
  console.log(`Universe: ${STOCKS.length} stocks (excluded: ${V8_EXCLUDED_STOCKS.join(", ")})`);
  console.log("======================================");
  console.log("");
  ...
  console.log("");
  console.log("======================================");
  console.log("V8 PORTFOLIO SUMMARY");
  console.log("======================================");
